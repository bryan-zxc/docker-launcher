...
- Fetch the ticket: `gh issue view <number> --repo bryan-zxc/docker-launcher`
...
For any ticket that changes the UI, consult `/fe-dev` for design tokens, component patterns, and Playwright testing. Create an HTML mockup **before** proceeding:
...
2. Start a local HTTP server in the `.mockups/` directory:
   ```bash
   python3 -m http.server 8765 --directory .mockups &>/dev/null &
   ```
...
3. On approval, implement the changes — for frontend work, follow `/fe-dev` conventions (CSS tokens, component classes, no emoji)

### 5. Test

Every ticket must be tested before completion.

**Frontend changes:** Use Playwright in headed mode to verify:
```bash
PLAYWRIGHT_MCP_SANDBOX=false playwright-cli open http://localhost:3000 --headed
```
Navigate to the feature, interact, take screenshots, and verify the expected behaviour. Both `--headed` and `PLAYWRIGHT_MCP_SANDBOX=false` are required — without them the browser is invisible to the user.

**Backend changes:** Run relevant backend tests. Unless the change is trivial, also do a frontend validation to confirm the backend change is reflected in the UI.
...
PARENT=$(gh api graphql -f query='
  query {
    repository(owner: "bryan-zxc", name: "docker-launcher") {
...
gh api graphql -f query='
  ...
  }' --jq '.data.repository.issue.subIssues.nodes[] | select(.projectItems.nodes[0].fieldValueByName.name | . == "Done" | not) | .number'
```
